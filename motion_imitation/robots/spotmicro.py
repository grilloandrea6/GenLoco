...
INIT_MOTOR_ANGLES = np.array([0, 0.7 ,-1.4] * NUM_LEGS)
_DEFAULT_TORQUE_LIMITS = [4, 4, 4] * 4
HIP_NAME_PATTERN = re.compile(r"\w+_shoulder")
UPPER_NAME_PATTERN = re.compile(r"\w+_leg")
LOWER_NAME_PATTERN = re.compile(r"\w+_foot")
TOE_NAME_PATTERN = re.compile(r"\w+toe")
IMU_NAME_PATTERN = re.compile(r"imu\d*")

URDF_FILENAME = "robot_descriptions/spotmicro_description/urdf/spotmicro.urdf"

_BODY_B_FIELD_NUMBER = 2
_LINK_A_FIELD_NUMBER = 3

class SpotMicro(base_robot.Base_robot):
  ...
  def GetActionLimit(self):
      return np.array([1.5] * NUM_MOTORS)